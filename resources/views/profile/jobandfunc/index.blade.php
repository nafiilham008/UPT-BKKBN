...
@section('title', __('Job and Function'))
...
@section('content')
    <div class="page-heading">
            ...
            <div class="row">
                <div class="col-12 col-md-8 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">
                        {{ __('Below is a list of job and function.') }}
                    </p>
                </div>
                <x-breadcrumb>
                    <li class="breadcrumb-item"><a href="{{ url('dashboard') }}">{{ __('Dashboard') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Job and Function') }}</li>
                </x-breadcrumb>
            </div>
        </div>
        ...
        <section class="section">
            ...
            @can('jobandfunc create')
                <div class="d-flex justify-content-end {{ $jobandfunc->count() == 1 ? 'd-none' : '' }}">
                    <a href="{{ route('dashboard.jobandfuncs.create') }}" class="btn btn-primary mb-3">
                        <i class="fas fa-plus"></i>
                        {{ __('Add Job and Function') }}
                    </a>
                </div>
            @endcan
                                    ...
                                    <tbody>
                                        @foreach ($jobandfunc as $item)
                                            <tr>
                                                ...
                                                @include('profile.jobandfunc.include.action')
